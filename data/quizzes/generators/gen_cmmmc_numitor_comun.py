...
def sync(a,b):
    m=lcm(a,b); ma=[a*i for i in range(1,m//a+1)]; mb=[b*i for i in range(1,m//b+1)]
    return iq(f"Șirul din {a} în {a} și șirul din {b} în {b} pornesc simultan. La ce număr se întâlnesc prima dată?","lcm_workbench",{"mode":"sync","a":a,"b":b,"multiples_a":ma,"multiples_b":mb,"answers":{"lcm":m}},f"Prima întâlnire este la {m}, adică c.m.m.m.c.({a},{b}).")
...
def build_questions():
    ...
    q += [lcm_lists(3,5),lcm_lists(6,8),lcm_lists(12,18),lcm_select(4,6,[8,12,16,18,24,30,36,48]),lcm_select(5,7,[20,28,35,42,70,105]),sync(6,8),sync(9,12),sync(10,15)]
    ...
    q += [common_den(a,b) for a,b in pairs]
    assert len(q)==40
    return q
def main():
